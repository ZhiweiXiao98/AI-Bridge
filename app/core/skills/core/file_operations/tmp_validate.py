...
from .result_utils import success_result, error_result, verify_contains
...
def replace_in_file(path: str, old_text: str, new_text: str, count: int = 1, atomic: bool = True, output_format: str = 'text', validate_code: bool = True, knowledge_engine=None):
    ...
    if not ok_guard:
        return error_result('replace_in_file', path, reason, output_format=output_format, Replacements=0, BlockedBySafetyGuard=True, TotalMatches=occurrences, **risk)
